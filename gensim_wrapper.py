...
class LdaTransformer(BaseEstimator):
    ...
    def __init__(self, n_latent_topics=100, use_tfidf=False, distributed=False, chunksize=2000, passes=1,
                 update_every=1, alpha='symmetric', eta=None, decay=0.5, eval_every=10, iterations=50,
    ...
    def fit(self, X, y=None):
        ...
        self.model = models.LdaModel(corpus, id2word=self.dictionary, num_topics=self.n_latent_topics,
    ...
    def get_params(self, deep=False):
        return {'n_latent_topics': self.n_latent_topics, 'distributed': self.distributed, 'chunksize': self.chunksize,
                'passes': self.passes, 'update_every': self.update_every, 'alpha': self.alpha, 'eta': self.eta,
                'decay': self.decay, 'eval_every': self.eval_every, 'iterations': self.iterations,
                'gamma_threshold': self.gamma_threshold}
...
class LsiTransformer(BaseEstimator):
    ...
    def __init__(self, n_latent_topics=100, use_tfidf=True, chunksize=20000, decay=1.0, distributed=False, onepass=True,
    ...
    def fit(self, X, y=None):
        ...
        self.model = models.LsiModel(corpus, id2word=self.dictionary, num_topics=self.n_latent_topics,
                                     chunksize=self.chunksize, decay=self.decay, distributed=self.distributed,
    ...
    def get_params(self, deep=False):
        return {'n_latent_topics': self.n_latent_topics, 'use_tfidf': self.use_tfidf, 'chunksize': self.chunksize,
                'decay': self.decay, 'distributed': self.distributed, 'onepass': self.onepass,
